...
## [0.0.13](https://github.com/OxideAV/oxideav-jpegxl/compare/v0.0.12...v0.0.13) - 2026-08-18
...
- round-382 (parent-dispatch r382) against ISO/IEC FDIS 18181-1:2021 — multi-frame codestream iteration: `decode_all_frames` / `decode_all_frames_from_codestream` walk the §C.1 frame array (shared prelude + byte-aligned FrameHeader/TOC/section groups, terminated by `is_last`); 2024-edition `RestorationFilter` (Table J.1) decode unblocking the animation-3frame first frame.
- round-375 (parent-dispatch r375) against ISO/IEC FDIS 18181-1:2021 — **drawn-image-feature gate on the registered Modular decode path (§5.2 / Table C.5).** A frame whose `FrameHeader.flags` carries kNoise (`0x01`, §K.4), kPatches (`0x02`, §K.2), or kSplines (`0x10`, §K.3) requires those features to be *drawn* after the restoration filters and before the colour transform (§5.2 decode order: IT → RF → features → colour transform). This crate does not yet render any of them, so emitting pixels would silently drop a visible part of the image — the exact "silent misparse" the crate forbids. New `reject_undrawn_image_features(flags)` helper returns a precise `Error::Unsupported` naming the requested feature(s) in §5.2 draw order (patches → splines → noise) and the raw `flags` value; it is wired into `decode_codestream` for the Modular branch. kUseLfFrame (`0x20`) and kSkipAdaptiveLFSmoothing (`0x80`) carry no separate draw step and are deliberately **not** gated. Three new unit tests cover the no-flag pass-through (incl. the two ungated bits), per-feature rejection, and the combined-flag draw-order listing. None of the committed fixtures set these bits, so every existing pixel-exact decode is unaffected.
- round-375 (parent-dispatch r375) against ISO/IEC FDIS 18181-1:2021 — **§J restoration filters wired into the registered Modular XYB decode path.** The integer→float dequant rescale (Listing L.2.2 preamble, `modular_xyb_rescale`) now produces three explicit XYB float planes that the §J.2 Gabor-like transform (`gaborish::apply_xyb_planes_in_place`, applied when `restoration_filter.gab`) and the §J.3 edge-preserving filter (`epf::apply_epf_iterations` with the §J.3.3 Modular constant sigma `epf_sigma_for_modular`, applied when `epf_iters > 0`) consume **in-place**, before the §L.2.2 inverse XYB colour transform — matching the §5.2 decode order (IT → RF → features → colour transform). `build_rgb_planes_from_xyb` is restructured into three stages (rescale → §J filters → §L.2.2 per-sample) and now takes the frame's `RestorationFilter`. With both flags clear the path is byte-for-byte identical to the pre-filter result (regression-pinned on a non-flat gradient). Two new tests: `xyb_restoration_filters_preserve_flat_field` (the §J.2 unit-sum kernel + §J.3 zero-distance weights both preserve a constant field, so a flat XYB image decodes identically filters-on vs filters-off and yields a spatially-constant RGB plane) and `xyb_filters_off_matches_unfiltered_gradient` (filters-off equals the old single-loop `modular_xyb_to_linear_rgb`). `ImageMetadataFdis::defaults()` promoted to `pub` as a baseline-metadata accessor for tests/tooling. The pure-lossless (non-XYB) pass-through and YCbCr modular paths are untouched, so the bit-exact small-fixture decodes are unaffected.
- round-372 (parent-dispatch r372) against ISO/IEC FDIS 18181-1:2021 — **VarDCT `vardct-256x256-d1` LF-magnitude divergence pinned as an exact *measurement* + HF-isolation localisation.** Rounds 362/367 *inferred* the d1 reconstruction's internal XYB Y plane was "≈ 4.0× too large" from a frame-mean comparison and localised the residual divergence to the LfQuant modular sub-bitstream decode. Round 372 converts that to a measurement: new `round372_vardct_lf_magnitude_ratio` (2 tests) decodes the *real* LfGroup of the committed fixture through the public LF primitives (`LfGroup::read` → `dequant_lf` with the Listing C.1 / F.1 multipliers), and independently inverts the `djxl` reference PNG (the validator's **opaque output** — its source never consulted) through the spec **forward** XYB transform (the default `OpsinInverseMatrix` inverse). The ratio of our dequantised LF **Y-mean** to the reference's forward-XYB Y-mean is **exactly 4.0** (measured: `global_scale = 5111`, `quant_lf = 17`, `extra_precision = 1`, `m_y_lf_unscaled = 512` → `m_y_dc = 0.005893`; decoded `qY` mean ≈ 622 → dequant Y mean ≈ 1.832 vs reference 0.458). The second test pins the Y plane is **shape-correct** — its `/4`-scaled values form a smooth luma-DC field (relative horizontal first-difference < 0.10 of the mean), which rules out a structural mis-decode and isolates the divergence to a scalar 4× on the modular-decoded LF quantities. A controlled HF-isolation experiment this round (env-gated, reverted; not committed) further showed that even with every HF AC coefficient zeroed and Y scaled by the measured 4×, the reconstruction still rails ~74 %: with HF zeroed the per-plane XYB means become Y = 0.458 (**exact** vs reference 0.458), X-mean ≈ 0.009 but with a ±0.5 swing where the reference X ≈ 0, and B ≈ 0.23 (reference 0.47, **≈ 2× undershoot**). So the residual railing is driven by the **X chroma DC plane swing** + the **B plane undershoot**, both the same modular-magnitude family as Y — not an IDCT / §C.5.4 placement / §6.2 crop / §L.2.2 XYB→RGB / chroma-from-luma artefact (`x_from_y`/`kX` are all-zero for this fixture, so the existing spatial HF chroma-from-luma is a no-op on X). **Docs gap (unchanged, now sharper):** pinning the exact per-token divergence inside the LfQuant modular decode requires a per-sample LF reference trace for `vardct-256x256-d1` — decoded `qX/qY/qB` against bit positions, all three channels (the X/B family confirms it is not Y-specific); the staged `trace.txt` carries only section-level bit checkpoints. No `src/` behaviour change this round — deliverable is the committed measurement test that replaces the round-362/367 inference + the X/B-plane sharpening of the localisation.
- round-367 (parent-dispatch r367) against ISO/IEC FDIS 18181-1:2021 — **corrected `vardct-256x256-d1` magnitude-bug localisation + DC-preservation invariant.** Round 362 localised the d1 reconstruction divergence (output rails ~99.8 % of samples; internal XYB Y plane ≈ 4.0× too large) to the coefficient-magnitude path and recorded, as supporting evidence, that the LF "DC passes the spec IDCT with **unit gain** (Annex I.2.1)" — reasoning that implicitly assumed DCT8×8 varblocks. Round 367 inspected the fixture's actual §C.5.4 `DctSelect` grid and found **every one of the 16 varblocks is DCT64×64** (Table C.16 numerical value 18 — 8×8 LF-block units tiling the full 32×32 LF grid), NOT DCT8×8. For DCT64×64 the LF→spatial path is the *non-trivial* §I.2.5 Listing I.16 chain (forward `DCT_2D` of the 8×8 LF samples × `ScaleF(8,64,·)` → §I.2.4 LLF prefix merge → §I.2.3.2 IDCT64×64), so the round-362 "unit-gain" note had to be re-justified for the real transform. New `round367_lf_to_llf_dc_preservation` integration test (2 tests) pins, as a permanent invariant, that this longer chain **does preserve DC magnitude exactly**: a flat LF block of value `V` reconstructs to a flat spatial block of `V` for DCT8×8 / 16×16 / 32×32 / 64×64 / 64×32 / 32×64 / 128×128 / 256×256, with an explicit `V = 1.83` (the measured d1 Y-plane LF mean) DCT64×64 case. Combined with round 362's confirmation that the Listing F.1 dequant, the Table C.12 Quantizer parse, and the Table C.11 `m_*_lf_unscaled` are spec-correct, this **rules out** the §I.2.5 LLF scaling, the §I.2.3.2 IDCT, the §C.5.4 placement, the §6.2 crop and the §L.2.2 XYB→RGB transform — leaving the **LfQuant modular sub-bitstream decode** (the decoded `qX/qY/qB` integers themselves) as the sole remaining suspect, consistent with the round-17 bisect's record that the LF per-sample loop over-consumes bitstream (`round17_d1_bit_trace` now shows 11592 vs the cjxl 11728-bit DC-group budget). Empirically (round-367 diag, since reverted): the d1 Y-channel LF quant samples have mean ≈ 622 / abs-max 826 (a smooth real-photo DC gradient, e.g. first row `[424, 476, 502, 503, …]`), dequantising to mean ≈ 1.83 against the spec-correct `m_y_dc ≈ 0.002946`; the reference inverts to ≈ 0.46, so the `qY` integers themselves are ≈ 4× too large. **Docs gap:** pinning the exact per-token divergence inside the LfQuant modular decode requires a per-sample LF reference trace for `vardct-256x256-d1` (decoded `qX/qY/qB` against bit positions); the staged `trace.txt` only carries section-level bit checkpoints. No `src/` behaviour change this round — deliverable is the corrected localisation + the DC-preservation invariant test.
- round-362 (parent-dispatch r362) against ISO/IEC FDIS 18181-1:2021 — **VarDCT reference-divergence harness + magnitude-bug localisation.** Round 355 left the integrated single-LfGroup VarDCT decode running the whole §C.8.3 → §L.2.2 chain end-to-end but withheld the public pixels because the per-block coefficient scaling was "not yet validated bit-exact against a reference decode" — without committing any reference to validate against. Round 362 commits that missing measurement: `tests/fixtures/vardct_256x256_d1_expected.png` (the opaque output of the `djxl` black-box validator on the existing `vardct_256x256_d1.jxl` fixture — its *source* never consulted) and `round362_vardct_d1_reference_divergence`, which decodes our integrated reconstruction (`decode_vardct_frame_from_codestream`) and pins the divergence at the RGB output level: the reference is an ordinary mid-tone photo (per-channel frame-means R≈127/G≈129/B≈139, <2 % railed) while our output **rails ~99.8 % of samples to 0/255** (MAD ~105–129/255) because the internal XYB magnitude is several times too large and clips through the non-linear Listing L.1 XYB→RGB step. The test is a forward ratchet — when the magnitude fix lands the saturation collapses and the assertion demands tightening toward a true pixel-accuracy gate. **Diagnosis** (traced this round against the reference inverted through the default `OpsinInverseMatrix`, no external source): the divergence is localised to the **coefficient-magnitude path**, NOT the IDCT / §C.5.4 placement / §6.2 crop / §L.2.2 XYB→RGB — each verified spec-conformant by inspection. The Y luma plane is ≈ 4.0× too large in the XYB domain (frame-mean Y ≈ 1.83 vs reference ≈ 0.46); Y carries no chroma-from-luma term (Annex G: `Y = dY`) and its DC passes the spec IDCT with unit gain (Annex I.2.1 `in[k] = out[0] + …`), so this is a clean LF-coefficient magnitude error upstream of the IDCT. The Listing F.1 dequant, the Table C.12 Quantizer parse (`global_scale`/`quant_lf`), and the Table C.11 `m_*_lf_unscaled` were each confirmed spec-conformant, leaving the LfQuant modular sub-bitstream decode as the prime suspect (a power-of-two factor is consistent with the clean 4×, and `lf_group.rs` already records an 11995-vs-11728-bit consumption discrepancy on the LfCoefficients section). The X chroma plane's oversized swing (reference X ≈ 0, ours ±2.6) is the same magnitude family on the X HF coefficients — for this fixture `x_from_y` is all-zero and `base_correlation_x == 0`, so the Annex G `kX` is exactly 0 and the swing is not a CfL artefact. Separately, the round confirmed (against the normative Annex G pipeline diagram: LF→DQ→CfL and HF→DQ→CfL→IT) that CfL is a **coefficient-domain** step applied before the IDCT with distinct LF (`x_factor_lf`) and HF (`XFromY`) factors, whereas the current `apply_chroma_from_luma` runs once on the post-IDCT spatial residual; this is latent for `vardct-256x256-d1` (`kX == 0`, default LF factors) but must be reworked before a multi-correlation fixture decodes correctly. No `src/` behaviour change this round — the deliverable is the committed validator-backed measurement + the localised next-step target. Restoration filters (Gaborish §J.2, EPF §J.3) and the non-empty-`lf_thresholds` `qdc_at` lookup remain pending.
- round-355 (parent-dispatch r355) against ISO/IEC FDIS 18181-1:2021 — **integrated single-LfGroup VarDCT decode reaches pixels.** Rounds 343/346/349 built every primitive of the per-LfGroup HF reconstruction (`reconstruct_lf_group_from_histogram`, `HfGlobalSection`, the §C.8.3 `PerPassHfHeaders`) but `decode_vardct_round13` still bailed `Unsupported` right after the §C.7 parse + Listing F.1/F.2 LF dequant, with the "remaining: §C.8.3 per-pass header reads + `qdc_at` LF lookup + `BlockContextResolver` history" deferral. Round 355 wires that remaining chain together in the integrated path (renamed `decode_vardct_frame`): it reads the §C.8.3 per-pass `hfp` header (`PerPassHfHeaders::read`) at the head of the PassGroup payload (continuing on the same bit cursor past HfGlobal in the single-TOC layout, or on the PassGroup slot reader in the multi-TOC layout), binds the §C.7.2 histograms to it (`HfGlobalSection::decode_context`), builds the `PerPassNonZerosGrids` (sized to the §C.5.4 DctSelect block grid) + the `BlockContextResolver` (over the LfGlobal §I.2.2 `HfBlockContext`) + the F.3 `DequantContext` (default dequant-matrix set + `OpsinInverseMatrix` + `QmScaleFactors::for_frame`), then drives `reconstruct_lf_group_from_histogram` → three XYB residual planes (HF entropy decode → cross-pass accumulate → F.3 dequant → §I.2.4 LLF merge → §I.2.3.2 IDCT → Annex G CfL), §6.2-crops them to the logical frame extent, and converts §L.2.2 inverse-XYB → 8-bit RGB. The integrated `qdc_at` supplies a zero quantised-LF DC triple, exact for any `HfBlockContext` with empty `lf_thresholds` (the §C.13 `block_context` derivation consumes `qdc` *only* through that ladder) — including the `vardct-256x256-d1` fixture's custom `nb_block_ctx=3` bundle; a bundle with non-empty `lf_thresholds` is rejected precisely (the per-varblock LF-DC lookup is the next wiring step). **Pixel-validation gate:** the whole §C.8.3 → §L.2.2 chain now *executes* on a real codestream (producing a shaped 256×256×3 RGB frame, which it never did before), but the per-block HF coefficient scaling is not yet validated bit-exact against a reference decode, so the *public* `decode_one_frame` path withholds the reconstructed pixels — it returns a precise "runs end-to-end but pixels not yet validated" `Error::Unsupported` rather than risk a silent misparse. The new `decode_vardct_frame_from_codestream` test/tool entry returns the reconstruction's pixels. New `round355_vardct_integrated_pipeline` integration tests (public-path end-to-end sentinel + integrated-frame shape: 3 planes, 256×256, stride 256); `round349_hf_global_section_parse` rewritten to the round-355 sentinel + a reconstruction-feeds-pixels assertion. Restoration filters (Gaborish §J.2, EPF §J.3) and the non-empty-`lf_thresholds` `qdc_at` lookup remain the documented next steps.

- round-355 follow-up — **LF channel-order reindex** in the integrated VarDCT path. The LfCoefficients modular sub-bitstream stores the three LF channels in JXL's native XYB *modular* order `(Y, X, B)` (the same "first three channels are Y', X', B'" convention §L.2.2 documents for kModular, per `xyb::modular_xyb_rescale`), whereas `dequant_lf` / `LfDequantOutput` are contractually `[X, Y, B]` (Listing F.1 applies `m_x_dc` to channel 0). `decode_vardct_frame` now maps modular index 1 → X-slot 0 and index 0 → Y-slot 1 (B unchanged), reindexing the quant samples + per-channel widths/heights. The X↔Y swap was latent because no integrated VarDCT path reached the §L.2.2 XYB→RGB step before this round; without the fix the large-magnitude luma plane was dequantised with the X multiplier and fed as chroma, collapsing the whole frame to a single constant colour (R=255,G=0,B=0). After the fix `vardct-256x256-d1` reconstructs to real photo content (82/188/171 distinct values across the R/G/B planes, full 0..255 range). New `round355` non-degeneracy test pins this (>16 distinct values + >64 range per plane). Exact pixels are still not asserted (per-block HF scaling unvalidated), so the public path still withholds them.

- round-349 (parent-dispatch r349) against ISO/IEC FDIS 18181-1:2021 — **§C.7 HfGlobal-section assembly** + integrated-path wiring. The HfGlobal TOC slot of a VarDCT frame (Table C.17 / §C.6) is three contiguous pieces on the same bit cursor with no byte alignment between them: §I.2.4 dequant matrices + §I.2.6 `num_hf_presets` (`HfGlobal::read`) → §C.7.1 `num_hf_presets` `HfPass` coefficient-order bundles (`read_hf_pass_sequence`) → §C.7.2 `495 × num_hf_presets × nb_block_ctx` HF-coefficient histograms (`HfCoefficientHistograms`) + the §C.3.2 ANS-state init (`u(32)`, no-op for prefix). Each primitive existed since rounds 90/133/247 but nothing chained them: `HfGlobal::read` stopped after piece 1 and `decode_vardct_round13` bailed `Unsupported` before pieces 2+3. New `hf_global_section::HfGlobalSection::read` performs all three reads in spec order (threading `nb_block_ctx` from the LfGlobal `HfBlockContext`, §I.2.2), producing a ready-to-decode `HfCoefficientHistograms` (post-`read_ans_state_init`) + the per-preset coefficient orders. `HfGlobalSection::decode_context(&PerPassHfHeaders)` binds those histograms to the per-frame §C.8.3 per-pass `hfp` / `histogram_offset` sequence to produce the `HfHistogramDecodeContext` that `reconstruct_lf_group_from_histogram` walks against — the README-named "construct the HfHistogramDecodeContext (its ANS-state init for ANS-coded histograms)" bridge — cross-validating every `hfp < num_hf_presets`. The integrated VarDCT decode path now parses through the full §C.7 section: `vardct_256x256_d1.jxl` advances past the HfPass + histogram reads to the new post-§C.7 `Unsupported` boundary (remaining: §C.8.3 per-pass header reads + `qdc_at` LF lookup + `BlockContextResolver` history feeding the reconstruction). 5 new `hf_global_section` lib unit tests (end-to-end chain on a 495-dist prefix histogram, cursor-position identity vs piecewise re-read, `nb_block_ctx==0` guard, `decode_context` bind + out-of-range-`hfp` rejection) + 1 new `round349_hf_global_section_parse` integration test pinning the on-real-bytes §C.7 advance.

- round-346 (parent-dispatch r346) against ISO/IEC FDIS 18181-1:2021 — **histogram-backed** multi-pass per-LfGroup VarDCT decode + reconstruction, closing the README-noted "§C.7.2 entropy-histogram materialisation that backs those closures" wiring step. Two new drivers: (1) `HfHistogramDecodeContext::decode_lf_group_multi_pass_three_channels` — the histogram-source sibling of the closure-based `multi_pass_decode::decode_multi_pass_three_channels_with_resolver`. It runs the full §C.8.3 outer-pass loop over the round-260 three-channel varblock walk, threading the per-pass per-channel `PredictedNonZeros` read (Listing C.13, before each varblock) + `NonZeros(x, y)` writeback (the `(raw + num_blocks - 1) Idiv num_blocks` line right after Listing C.14, after each varblock) through `PerPassNonZerosGrids`, and **owns the §C.7.2 entropy-stream routing itself** via the round-252 per-pass `histogram_offset` array — the only entropy input the caller still supplies is the storage-only `qdc_at(p, &vb)` quantised-LF lookup, no `read_non_zeros` / `decode_symbol` closures. (2) `vardct_reconstruct::reconstruct_lf_group_from_histogram` — fuses that decode with the round-340 cross-pass reconstruction (accumulation → F.3 dequant → §I.2.4 LLF merge → §I.2.3.2 IDCT → §C.5.4 placement → Annex G CfL) in one call, the histogram-source sibling of round-343's closure-based `reconstruct_lf_group_from_entropy`. Pass-count cross-validation is layered: `reconstruct_lf_group_from_histogram` rejects `nz.num_passes() != passes.num_passes` up front, and the inner driver independently rejects `nz.num_passes() != ctx.num_passes()` (the histogram-offset table bound) before consuming any stream bytes. Both new drivers delegate to already-tested primitives so the histogram path and the closure path cannot diverge. New `round346_lf_group_multi_pass_histogram` integration test (8 tests) pins: single-pass + two-pass raster/short-circuit + pass-count rejection, `qdc_at` observing the (pass, raster) order, **bit-for-bit identity** of the histogram-backed driver against the closure path wired to an identical histogram context over the same §C.7.2 stream (`coeffs` + `coeffs_read` + `raw` per channel + equal bits consumed), end-to-end 2×2 DCT8×8 flat reconstruction, and `to_bits()`-exact match of the fused one-call histogram path against the explicit decode-then-reconstruct two-call path on a two-pass non-square DCT8×16 frame. 2 new lib unit tests in `multi_pass_hf_histogram_decoder`. 76 test binaries green workspace-wide
- round-343 (parent-dispatch r343) against ISO/IEC FDIS 18181-1:2021 — fused **live-entropy** per-LfGroup VarDCT reconstruction: `vardct_reconstruct::reconstruct_lf_group_from_entropy` runs the §C.8.3 multi-pass entropy decode (`multi_pass_decode::decode_multi_pass_three_channels_with_resolver`, the per-pass / per-varblock / per-channel walk) AND the round-340 cross-pass reconstruction in a single call per LfGroup, closing the README-noted "feed it the live per-pass `DecodedHfBlock` stack from the §C.7.2 entropy stream rather than a caller-supplied one" wiring step. The entropy side is the exact `qdc_at(p, &vb)` / `read_non_zeros(p, c, predicted)` / `decode_symbol(p, c, coeff_ctx)` closure triple the decode driver already takes (so the §C.7.2 histogram array stays a caller concern); the reconstruction inputs (`grid` / `lf` / `dq` / CfL) match `reconstruct_lf_group_cross_pass` verbatim. A `PerPassNonZerosGrids` pass count disagreeing with `Passes.num_passes` is rejected up front, before any entropy work. The fused driver delegates both halves to the already-tested primitives so the two paths cannot diverge. New tests pin: all-zero-entropy single-pass DCT8×8 flat reconstruction, **bit-for-bit identity** (`to_bits()` per sample) of the fused one-call path against the explicit decode-then-reconstruct two-call path, and the pass-count-mismatch rejection. New `round343_from_entropy_reconstruct` integration test drives a 2×2 DCT8×8 grid + a two-pass non-square DCT8×16 frame end to end through the live walk (fused == explicit, bit-exact) and asserts an entropy-side closure error propagates verbatim out of the fused call. 856 lib tests green (3 new) + 3 new integration tests
- round-336 (parent-dispatch r336) against ISO/IEC FDIS 18181-1:2021 — §J.3.3 VarDCT **per-block-sigma** EPF driver: `epf::apply_epf_iterations_per_block_sigma` generalises round-332's constant-sigma `apply_epf_iterations` to the kVarDCT case, where §J.3.3 sets `sigma` from "the values in HfMul and Sharpness (C.5.4) at the coordinates of the 8 × 8 rectangle containing the reference pixel" via Listing J.3. New `epf::SigmaGrid` packs the per-8×8-block Listing J.3 sigma values (`blocks_w × blocks_h`, row-major; `at_pixel(x, y)` maps to block `(x/8, y/8)` with a defensive last-block clamp; constructor rejects wrong length / non-finite / `<= 0` entries since Listing J.3 clamps `sigma >= 1e-4`). New `epf::apply_step_per_block_sigma` is the per-pixel-sigma kernel applier: it looks up each reference pixel's block sigma and either (a) **skips** the pixel — output == input — when the block sigma is `< epf::EPF_SKIP_SIGMA` (0.3; Listing J.3 tail "If sigma < 0.3 for a given varblock, the decoder skips all steps on the pixels of that block"), or (b) filters with `inv_sigma` from that block's sigma. §J.3.1 pass dispatch / scalar sourcing / scratch ping-pong are identical to the constant-sigma driver. New tests pin: SigmaGrid length + non-positive-entry rejection, `at_pixel` block mapping + trailing-block clamp, a **uniform grid reducing bit-exactly to the constant-sigma `apply_epf_iterations`** for iters 1..=3, all-sub-0.3 grid leaving the planes untouched (full skip), a mixed grid passing low-sigma blocks through while filtering the rest, epf_iters==0 no-op, epf_iters>3 rejection, and constant-plane fixed-point. 52 epf tests green (8 new); 73 test binaries green workspace-wide

- round-332 (parent-dispatch r332) against ISO/IEC FDIS 18181-1:2021 — §J.3.1 three-step EPF iteration driver: `epf::apply_epf_iterations` composes the up-to-three §J.3 passes (round-144's `apply_step_13tap` for Step 0, `apply_step_5tap` for Steps 1/2) sequentially with the output of each step feeding the next (§J.3.4 "The output of each step of the filter is used as an input for the following step"), landing the per-frame EPF wiring the round-144 module comment deferred for the **constant-sigma (Modular) case** (§J.3.3 sets `sigma = rf.epf_sigma_for_modular` for every pixel, so the per-varblock Listing J.3 sigma table + the `sigma < 0.3` skip do not apply). Pass dispatch follows §J.3.1: `epf_iters` 0→no-op, 1→Step 1, 2→Steps 1+2, 3→Steps 0+1+2; `epf_iters > 3` rejected as a malformed `u(2)` Table C.9 field. Per-pass scalars sourced per the module's documented Listing J.2 readings — Step 0 `step_multiplier = epf_pass0_sigma_scale` / `zeroflush = epf_pass1_zeroflush` (compressed step 0), Step 1 `step_multiplier = 1.0` / same zeroflush, Step 2 `step_multiplier = epf_pass2_sigma_scale` / `zeroflush = epf_pass2_zeroflush` (compressed step 1); `epf_border_sad_mul` + `epf_channel_scale` thread through every pass. Filters the three XYB planes in place via a scratch-triple ping-pong (each §J.3.4 pass is out-of-place; odd pass count copies the final scratch back). New tests pin: epf_iters==0 identity, constant-plane fixed-point for iters 1..=3, epf_iters>3 + length-mismatch rejection, zero-area no-op, and exact equivalence of the 1-pass / 2-pass driver output against hand-chained `apply_step_5tap` calls (pins ordering + buffer handoff). 821 lib tests green
- round-328 (parent-dispatch r328) against ISO/IEC FDIS 18181-1:2021 — §6.2 right/bottom crop of the padded VarDCT reconstruction: `residual_plane::ResidualPlane::crop_to` + `ChannelResidualPlanes::crop_to`. Rounds 306..322 produced the per-LfGroup XYB residual planes on the **padded block grid** (`width_blocks·8 × height_blocks·8`, `width_blocks = ceil(lf_w / 8)` per §C.5.4), and every reconstruct driver's doc comment ended with "the caller crops to `lf_w × lf_h`" — but no crop primitive existed. §6.2 (Group splitting) is the normative source: "The decoder ensures the decoded image has the dimensions specified in SizeHeader by cropping at the right and bottom as necessary." `crop_to(target_width, target_height)` keeps the top-left rectangle and discards the right/bottom padding (no resampling / edge handling), shrink-only (rejects a target that grows or is zero), with a copy-free fast path when the logical extent is already an exact multiple of 8. `ChannelResidualPlanes::crop_to` applies the same target to all three XYB planes
- round-322 (parent-dispatch r322) against ISO/IEC FDIS 18181-1:2021 — LF-aware per-LfGroup VarDCT three-channel residual-plane reconstruction: `residual_plane::assemble_three_channel_planes_with_lf` + `reconstruct_three_channel_planes_with_lf` wire the round-316 LLF-aware per-block decode into the round-306/309 spatial placement + Annex G chroma-from-luma stack. For each varblock per channel the driver extracts that channel's `cy × cx` LLF block from the LfGroup's dequantised `LfDequantOutput` (via `vardct::compose_lf_to_llf_block` at the varblock's block-grid origin) and threads it into the caller's `decode_with_llf(channel, vb, &llf)` closure (`block_dequant::decode_block_to_residual_with_llf`), so the assembled XYB planes now carry the full LF + HF spatial residual. Mismatched LF channel dims (the subsampled case, which must drive `compose_lf_to_llf_block` per channel) are rejected up front before any decode work
