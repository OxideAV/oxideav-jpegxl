...
//!
//! The state-evolution defect this chain was built to localise was
//! fixed in round 278 (`modular_fdis::wp_predict`: Listing E.2
//! `error2weight` inner-Idiv-first operand order + `true_errNW →
//! true_errN` fallback at x == 0 — see `r32_noise_bisect.rs`). The
//! cross-row reads at sample 194 now resolve to the spec values
//! (737 / -456 / -165), sample 194 decodes the spec value 34, and
//! the in-row chain runs against the spec decoded values throughout.
//! The historical narrative below documents the localisation work.
//!
...
///
/// Restricted to samples 192..=194 (the window the round-202 chain
/// was built around). From round 278 onward every sample decodes the
/// spec value, so `v_prod` == the trace doc's `true_value` at every
/// boundary (pre-fix, sample 194 decoded the round-32-pinned 35 and
/// samples 195+ cascaded into wrong MA-tree leaf picks).
#[test]
#[serial]
fn r202_row3_in_row_te_w_chain_192_to_194() {
    eprintln!("[r202] row-3 in-row te_w chain (samples 192..=194):");

    // Production decoded values at each pinned row-3 sample. All
    // spec-exact from round 278 onward (pre-fix, sample 194 decoded
    // 35 → tv8 = 280, the round-32 pixel divergence).
    fn v_prod_at(s: u32) -> i32 {
        match s {
            192 => 1192, // spec tv8 = 1192, v_spec = 149
            193 => 600,  // spec tv8 = 600,  v_spec = 75
            194 => 272,  // spec tv8 = 272,  v_spec = 34
...
/// Pin the production decoded sample value at sample 194 to the spec
/// value 34. Pre-round-278 production decoded 35 here (the round-32
/// first-pixel-divergence, caused by the WP state-evolution defect at
/// upstream samples per the round-191 / round-195 chain analysis).
#[test]
#[serial]
fn r202_sample_194_decoded_value_is_34_spec_exact() {
    ...
    assert_eq!(
        v_prod_at_194, 34,
        "Sample 194 must decode the spec value 34 from round 278 \
         onward (pre-fix production decoded 35 — the round-32-pinned \
         first-pixel-divergence).",
    );
}
...
fn r202_sample_194_cross_row_te_reads() {
    ...
    // te_nw@194 reads sample 129: this was the SMOKING GUN — pre-fix
    // production stored 716 (Δ = -21), documented at length in
    // `r191_pin_state_evolution_gap` and
    // `r195_sample_193_te_n_equals_sample_194_te_nw`. Spec-exact from
    // round 278 onward.
    assert_eq!(
        te_nw_prod,
        row2_spec::TRUE_ERR_AT_129,
        "te_nw at sample 194 (reads sample 129) must match spec ({}); \
         pre-round-278 production stored 716 (Δ = -21).",
        row2_spec::TRUE_ERR_AT_129,
    );

    // te_ne@194 reads sample 131: pre-fix production stored -160
    // (Δ = +5). Spec-exact from round 278 onward.
    assert_eq!(
        te_ne_prod,
        row2_spec::TRUE_ERR_AT_131,
        "te_ne at sample 194 (reads sample 131) must match spec ({}); \
         pre-round-278 production stored -160 (Δ = +5).",
        row2_spec::TRUE_ERR_AT_131,
    );
}
